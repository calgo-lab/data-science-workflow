#!/usr/bin/env bash
# Publish DSA Lecture 8 to the website + refresh the slides download.
#
...
#     ./publish-lecture-08.sh
# It uses the outputs already stored in the notebook; it does NOT re-execute.
#
# Code inputs are hidden EXCEPT the SVD/MF and AutoEncoder cells (those are
# tagged so nbconvert keeps them visible). Slides use the CENTERED scrollable
# layout (Session 8+); Sessions 6/7 stay left-aligned for low-res projectors.
set -euo pipefail
cd "$(dirname "$0")"
...
SRC="DSA/lecture-08.ipynb"
DST_DIR="DSA-sessions"
DST_BASE="session8_matrix_factorization"
...
echo "-> slides    $DST_DIR/$DST_BASE.slides.html  (code inputs hidden except SVD & AutoEncoder)"
