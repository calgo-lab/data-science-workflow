#!/usr/bin/env bash
# Publish DSA Lecture 9 to the website + refresh the slides download.
#
...
#     ./publish-lecture-09.sh
# It uses the outputs already stored in the notebook; it does NOT re-execute.
#
# Code inputs are hidden EXCEPT the two-step SBERT load+encode cell (left
# untagged on purpose so students see the API). Slides use the CENTERED
# scrollable layout (Session 8+); Sessions 6/7 stay left-aligned.
set -euo pipefail
cd "$(dirname "$0")"
...
SRC="DSA/lecture-09.ipynb"
DST_DIR="DSA-sessions"
DST_BASE="session9_neural_embeddings"
...
echo "-> slides    $DST_DIR/$DST_BASE.slides.html  (code inputs hidden except the SBERT encode cell)"
